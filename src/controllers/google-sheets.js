...
function authorize(credenciales, callback, data) {
  ...
  const clientSecret = credenciales.installed.client_secret;
  const clientId = credenciales.installed.client_id;
  const redirectUrl = credenciales.installed.redirect_uris[0];
...
function getNewToken(oauth2Client, callback, data) {
  ...
  rl.question('Ingresar el codigo para autorizar al cliente: ', (code) => {
    rl.close();
...
function addNewUser(auth, data) {
  ...
  sheets.spreadsheets.values.append(opciones, (err, res) => {
    if (err) {
